...
Normal runs update Homebrew metadata once and install/upgrade only selected packages and their necessary dependencies. No blanket upgrade, cleanup or removal occurs. `--no-upgrade` disables explicit metadata/package/toolchain upgrades and automatic Brew updates; installing missing packages may still resolve their required dependencies.
