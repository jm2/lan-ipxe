...
foreach ($Target in $Targets) {
    ...
    foreach ($Device in $Target.Devices) {
        ...
        try {
            ...
            $Manifest.Add("SKIPPED: $RTLName (search request failed)")
            continue
        }
        ...
        if (-not $UpdateIds) {
            ...
            $Manifest.Add("SKIPPED: $RTLName (no catalog candidates)")
            continue
        }
    ...
    foreach ($Group in $GroupedPackages) {
        ...
        try {
            ...
            Write-Warning "Download dialog request failed for $RTLName [$Arch]: $_"
            $Manifest.Add("SKIPPED: $RTLName [$Arch] (download dialog failed)")
            continue
        }
        ...
        if (-not $CabUrl) {
            ...
            $Manifest.Add("SKIPPED: $RTLName [$Arch] (no .cab URL in payload)")
            continue
        }
        ...
        try {
            ...
            Write-Warning "CAB download failed for $RTLName [$Arch]: $_"
            $Manifest.Add("SKIPPED: $RTLName [$Arch] (download failed)")
            continue
        }
        ...
        if ($sig.Status -ne 'Valid') {
            Write-Warning "Authenticode signature for $RTLName [$Arch] CAB is '$($sig.Status)' (not Valid) — skipping extraction/injection."
            $Manifest.Add("SKIPPED: $RTLName [$Arch] (signature $($sig.Status))")
        ...
        if ($expandProc.ExitCode -ne 0) {
            Write-Warning "expand.exe exited with code $($expandProc.ExitCode) for $RTLName [$Arch] — keeping source CAB '$CabFile' for retry/inspection."
            $Manifest.Add("SKIPPED: $RTLName [$Arch] (expand.exe exit $($expandProc.ExitCode))")
            continue
        }
        ...
        if ($infCount -gt 0 -and $sysCount -gt 0) {
            $Manifest.Add("ACQUIRED: $RTLName [$Arch]")
        }
        else {
            $Manifest.Add("SKIPPED: $RTLName [$Arch] (no .inf/.sys after extract)")
        }
