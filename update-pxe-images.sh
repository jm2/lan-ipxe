...
# --- Manjaro ---
get_manjaro_version() {
    # Manjaro scraping via Purdue is unreliable.
    # Manjaro doesn't have a standard "latest" link in many mirrors.
    # We will fallback to a hardcoded recent version or netboot.xyz.
    echo "26.0"
}
get_manjaro_iso() {
    # Manjaro filenames rely on date codes we can't guess without scraping.
    # Since Purdue mirror lacks ISOs, we can't return a valid one.
    echo "" 
}
...
if [ "$ENABLE_TFTP_BOOTSTRAP" = "true" ]; then
    ...
    curl -sL -o "$TFTP_DIR/ipxe.efi" "http://boot.ipxe.org/x86_64-efi/ipxe.efi"
    ...
    if curl -sI "http://boot.ipxe.org/arm64-efi/ipxe.efi" | head -n 1 | grep -q "200"; then
        ...
        curl -sL -o "$TFTP_DIR/ipxe-arm64.efi" "http://boot.ipxe.org/arm64-efi/ipxe.efi"
    else
        echo "  [WARNING] Could not find stock arm64 ipxe.efi. Skipping."
    fi
else
