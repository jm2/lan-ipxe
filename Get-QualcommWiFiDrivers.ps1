...
foreach ($Target in $Targets) {
    ...
    # Group all valid candidate packages into their correct respective HWModelId families, and find the freshest.
    $GroupedPackages = $AvailablePackages | Group-Object ModelId
    
    foreach ($Group in $GroupedPackages) {
        # Check if any packages exactly match the preferred major.minor branch 
        $TargetBranch = $Group.Group[0].PreferredBranch
        ...
        if ($BranchMatches.Count -gt 0) {
            ...
            Write-Host "   -> ModelId $($Group.Name): Selected v$($BestPackage.Version) [Preferred Branch] (Update ID: $($BestPackage.Id))" -ForegroundColor Green
        }
        else {
            $BestPackage = $Group.Group | Sort-Object Version -Descending | Select-Object -First 1
            Write-Host "   -> ModelId $($Group.Name): Selected v$($BestPackage.Version) [Global Highest] (Update ID: $($BestPackage.Id))" -ForegroundColor Green
        }
        ...
        $ExtractDir = Join-Path $TempDir "$($Target.Name)\$($BestPackage.ModelName)"
