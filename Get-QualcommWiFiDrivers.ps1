    ...
    foreach ($Device in $Target.Devices) {
        ...
        foreach ($Query in $Device.Queries) {
            ...
            foreach ($result in $DetailResults) {
                # The marketing-name queries (Killer AX500, FastConnect 6900, ...) also return
                # the combo-chip's Bluetooth/UART driver entries, which would otherwise pass the
                # lone -notmatch 'NDIS' filter. Exclude Title 'bluetooth'/'uart' so only the
                # Wi-Fi NIC package is selected.
