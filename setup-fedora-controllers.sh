...
shared MongoDB) plus a shadowsocks-rust server, as Podman quadlets, with
...
All components default to enabled. Each can also be set from the environment
(ENABLE_OMADA / ENABLE_UNIFI / ENABLE_SHADOWSOCKS = 0|1); arguments override
the environment. MongoDB (unifi-db) is provisioned automatically whenever
omada or unifi is enabled - and looks identical either way - but not at all
otherwise. Disabling a component on a re-run stops it and removes its
quadlet; data directories and images are kept for manual cleanup.
...
for flag in ENABLE_OMADA ENABLE_UNIFI ENABLE_SHADOWSOCKS; do
...
log "Components: omada=${ENABLE_OMADA} unifi=${ENABLE_UNIFI} mongodb=${ENABLE_MONGO} shadowsocks=${ENABLE_SHADOWSOCKS}"
...
============================================================
 Done. Components: omada=${ENABLE_OMADA} unifi=${ENABLE_UNIFI} mongodb=${ENABLE_MONGO} shadowsocks=${ENABLE_SHADOWSOCKS}
